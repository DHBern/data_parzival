...
module namespace model="http://www.tei-c.org/pm/models/shakespeare/fo";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(p) return
                        if (ancestor::sp) then
                            fo:inline($config, ., css:get-rendition(., ("tei-p1", css:map-rend-to-class(.))), .)
                        else
                    ...
                    case element(stage) return
                        if (contains(@rend, 'rightJustified')) then
                            fo:block($config, ., ("tei-stage1", css:map-rend-to-class(.)), .)
                        else
                            if (contains(@rend, 'center')) then
                                fo:block($config, ., ("tei-stage2", css:map-rend-to-class(.)), .)
                            else
                                fo:block($config, ., ("tei-stage3", css:map-rend-to-class(.)), .)
                    ...
                    case element(speaker) return
                        fo:inline($config, ., ("tei-speaker", css:map-rend-to-class(.)), .)
