...
module namespace model="http://www.tei-c.org/pm/models/vangogh/fo";
            ...
            return
                    ...
                    case element(lb) return
                        if ($parameters?view='normalized') then
                            ...
                            fo:break($config, ., css:get-rendition(., ("tei-lb3", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(TEI) return
                        fo:document($config, ., ("tei-TEI2", css:map-rend-to-class(.)), (teiHeader,.//div[@type='original'], .//div[@type='translation']))
                    case element(anchor) return
                        fo:note($config, ., ("tei-anchor", css:map-rend-to-class(.)), let $n := @n return $get(.)/ancestor::TEI//div[@type='notes']//note[@n=$n]/node(), (), @n/string())
                    ...
                    case element(note) return
                        if ($parameters?view='notes') then
                            fo:listItem($config, ., ("tei-note1", css:map-rend-to-class(.)), ., @n)
                        else
                    ...
                    case element(div) return
                        if (@type='original') then
                            (
                                fo:heading($config, ., ("tei-div1", css:map-rend-to-class(.)), 'Original', 2),
                            ...
                            if (@type='translation') then
                                (
                                    fo:heading($config, ., ("tei-div3", css:map-rend-to-class(.)), 'Translation', 2),
                                    fo:block($config, ., ("tei-div4", css:map-rend-to-class(.)), .)
                                )

                            else
                                if ($parameters?view='notes') then
                                    fo:list($config, ., ("tei-div5", css:map-rend-to-class(.)), for $anchor in .//anchor/@n return  root($parameters?root)//div[@type='notes']//note[@n=$anchor], 'custom')
                                else
                                    if (@type='textualNotes') then
                                        fo:omit($config, ., ("tei-div6", css:map-rend-to-class(.)), .)
                                    else
                                        if (@type='title_page') then
                                            fo:block($config, ., ("tei-div7", css:map-rend-to-class(.)), .)
                                        else
                                            if (parent::body or parent::front or parent::back) then
                                                fo:section($config, ., ("tei-div8", css:map-rend-to-class(.)), .)
                                            else
                                                fo:block($config, ., ("tei-div9", css:map-rend-to-class(.)), .)
                    ...
                    case element(ab) return
                        if ((ancestor::div[@type='translation'] or $parameters?view='normalized') and @rend='indent') then
                            fo:paragraph($config, ., ("tei-ab3", css:map-rend-to-class(.)), .)
                        else
                            if (ancestor::div[@type='translation'] or $parameters?view='normalized') then
                                fo:paragraph($config, ., ("tei-ab4", css:map-rend-to-class(.)), .)
                            else
                                if (@rend='indent') then
                                    fo:inline($config, ., ("tei-ab5", css:map-rend-to-class(.)), .)
                                else
                                    fo:inline($config, ., ("tei-ab6", css:map-rend-to-class(.)), .)
                    ...
                    case element(c) return
                        if (@type='kk') then
                            fo:inline($config, ., ("tei-c1", css:map-rend-to-class(.)), ',')
                        else
                            if (@type='kp') then
