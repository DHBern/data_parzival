...
module namespace model="http://www.tei-c.org/pm/models/ebbe/fo";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(teiHeader) return
                        fo:omit($config, ., ("tei-teiHeader3", css:map-rend-to-class(.)), .)
                    ...
                    case element(ref) return
                        if (starts-with(@target, 'EBBE_')) then
                            fo:link($config, ., ("tei-ref1", css:map-rend-to-class(.)), ., 'https://www.estreicher.uj.edu.pl/staropolska/baza/wpis/?sort=nazwisko_imie&amp;order=1&amp;id=' || substring-after(@target, 'EBBE_'), map {"target": '_ebbe'})
                        else
                            if (parent::cit or parent::def) then
                                (: references (authors) within translations, examples or definitions :)
                                fo:inline($config, ., ("tei-ref2", "ref-author", css:map-rend-to-class(.)), .)
                            else
                                if (not(@target)) then
                                    fo:alternate($config, ., ("tei-ref3", css:map-rend-to-class(.)), ., ., 'odsyłacz niezidentyfikowany')
                                else
                                    if (not(node())) then
                    ...
                    case element(ab) return
                        if (starts-with(@facs, 'iiif:')) then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-ab1", "ab", css:map-rend-to-class(.)), $content)
                        else
                            fo:block($config, ., ("tei-ab2", "ab", css:map-rend-to-class(.)), .)
                    ...
                    case element(title) return
                        if ($parameters?mode='metadata') then
                            ...
                            if ($parameters?header='short') then
                                fo:heading($config, ., ("tei-title2", css:map-rend-to-class(.)), ., 5)
                            else
                                if (parent::titleStmt/parent::fileDesc) then
                                    (
                                        if (preceding-sibling::title) then
                                            fo:text($config, ., ("tei-title3", css:map-rend-to-class(.)), ' — ')
                                        else
                                            (),
                                        fo:inline($config, ., ("tei-title4", "foo", css:map-rend-to-class(.)), .)
                                    )
                                            ...
                                            if (@level='s' or @level='j') then
                                                ...
                                                if (@level='u' or @level='a') then
                                                    (
                                                        ...
                                                        if (following-sibling::* and     (    ancestor::biblFull)) then
                                                            fo:text($config, ., ("tei-title11", css:map-rend-to-class(.)), '. ')
                                                        else
                                                            ()
                                                    )

                                                else
                                                    fo:inline($config, ., ("tei-title12", css:map-rend-to-class(.)), .)
                    ...
                    case element(cit) return
                        if (@type='example') then
                            fo:block($config, ., ("tei-cit1", "example", css:map-rend-to-class(.)), .)
                        else
                            if (@type='translation') then
                                fo:block($config, ., ("tei-cit2", "translation", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                        ...
                        if ($parameters?mode='metadata') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    ...
                                    fo:block($config, ., ("tei-fileDesc5", "sample-description", css:map-rend-to-class(.)), ../profileDesc/abstract)
                                )

                            else
                                fo:title($config, ., ("tei-fileDesc6", css:map-rend-to-class(.)), titleStmt)
                    ...
                    case element(quote) return
                        if (ancestor::cit) then
                            ...
                            if (ancestor::p) then
                                ...
                                fo:inline($config, ., css:get-rendition(., ("tei-quote2", css:map-rend-to-class(.))), .)
                            else
                                (: If it is inside a paragraph then it is inline, otherwise it is block level :)
                    ...
                    case element(gram) return
                        fo:inline($config, ., ("tei-gram", css:map-rend-to-class(.)), .)
                    case element(oRef) return
                        (: marker for headword reference; rendered as dash :)
                        fo:inline($config, ., ("tei-oRef", css:map-rend-to-class(.)), .)
                    case element(gen) return
                        fo:alternate($config, ., ("tei-gen", css:map-rend-to-class(.)), ., ., @norm)
                    case element(pos) return
                        fo:alternate($config, ., ("tei-pos", css:map-rend-to-class(.)), ., ., @norm)
                    case element(editor) return
                        if (ancestor::teiHeader) then
                            fo:block($config, ., ("tei-editor1", css:map-rend-to-class(.)), .)
                        else
                            fo:inline($config, ., ("tei-editor2", css:map-rend-to-class(.)), .)
                    case element(msDesc) return
                        (
                            (: retrieves the primary language :)
                            fo:block($config, ., ("tei-msDesc1", css:map-rend-to-class(.)), (root(.)//langUsage/language)),
                            (: retrieves the form of the source from taxonomy file :)
                            fo:block($config, ., ("tei-msDesc2", css:map-rend-to-class(.)), id(substring-after(root(.)//catRef[@scheme='#form']/@target, '#'), doc('/db/apps/tei-publisher/data/taxonomy.xml'))),
                            fo:block($config, ., ("tei-msDesc3", css:map-rend-to-class(.)), .)
                        )

                    case element(institution) return
                        fo:inline($config, ., ("tei-institution", css:map-rend-to-class(.)), .)
                    case element(respStmt) return
                        ...
                        return
                                                fo:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-respStmt", css:map-rend-to-class(.)), $content)
                    case element(idno) return
                        (
                            fo:inline($config, ., ("tei-idno1", css:map-rend-to-class(.)), .),
                            fo:inline($config, ., ("tei-idno2", css:map-rend-to-class(.)), ancestor::msDesc/@type)
                        )
