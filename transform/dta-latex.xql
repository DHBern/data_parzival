...
module namespace model="http://www.tei-c.org/pm/models/dta/latex";
            ...
            return
                    ...
                    case element(pb) return
                        if (count(../*) = 1 and count(ancestor::*) = 1) then
                            latex:inline($config, ., css:get-rendition(., ("tei-pb2", css:map-rend-to-class(.))), '[Empty page]')
                        else
                    ...
                    case element(fw) return
                        latex:omit($config, ., ("tei-fw6", css:map-rend-to-class(.)), .)
