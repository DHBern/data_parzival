...
module namespace model="http://www.tei-c.org/pm/models/docx-output/web";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(supplied) return
                        html:inline($config, ., ("tei-supplied", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(anchor) return
                        if (@type='note') then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-anchor1", "note", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(note) return
                        if (@target) then
                            html:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., (), let $nr := count(./preceding::note[@target]) let $ch := codepoints-to-string(string-to-codepoints("a") + $nr mod 26) return  '-' || $ch)                            => model:map($node, $trackIds)
                        else
                            if (@place) then
                                ...
                                if (parent::table) then
                                    ...
                                    if (parent::lg) then
                                        ...
                                        if (parent::list) then
                                            ...
                                            if (parent::div) then
                                                html:heading($config, ., ("tei-head6", css:map-rend-to-class(.)), ., count($get(.)/ancestor::div))                                                => model:map($node, $trackIds)
                                            else
                    ...
                    case element(placeName) return
                        if (@rend='smallcaps') then
                            html:link($config, ., ("tei-placeName1", css:map-rend-to-class(.)), ., 'https://www.google.de/maps/place/' || @ref, '_blank', map {})                            => model:map($node, $trackIds)
                        else
                            html:inline($config, ., ("tei-placeName2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    case element(persName) return
                        if (@ref) then
                            html:link($config, ., ("tei-persName1", css:map-rend-to-class(.)), ., @ref, '_blank', map {})                            => model:map($node, $trackIds)
                        else
                            html:inline($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    case element(tag) return
                        html:inline($config, ., ("tei-tag", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
