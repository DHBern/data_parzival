...
module namespace model="http://www.tei-c.org/pm/models/dantiscus2/print";
            ...
            return
                            ...
                            if ($parameters?mode='norm' and abbr and expan) then
                                ...
                                if (abbr and expan) then
                                    ...
                                    if (orig and reg) then
                                        printcss:alternate($config, ., ("tei-choice7", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt1", "editors", css:map-rend-to-class(.)), $content)
                        else
                            if ($parameters?header='short') then
                                (
                                    ...
                                    (: incipit :)
                                    html:block($config, ., ("tei-titleStmt7", "incipit", css:map-rend-to-class(.)), root(.)//seg[@type='incipit'])
                                )
                    ...
                    case element(profileDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc1", css:map-rend-to-class(.)), $content)
                        else
                    ...
                    case element(name) return
                        printcss:alternate($config, ., ("tei-name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node())
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", css:map-rend-to-class(.)), .)
                    case element(rs) return
                        printcss:alternate($config, ., ("tei-rs", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node())
