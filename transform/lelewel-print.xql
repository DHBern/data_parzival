...
module namespace model="http://www.tei-c.org/pm/models/lelewel/print";
            ...
            return
                    ...
                    case element(lb) return
                        if (@break) then
                            ...
                            html:break($config, ., css:get-rendition(., ("tei-lb3", "lb", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(choice) return
                        if (orig and seg) then
                            printcss:alternate($config, ., ("tei-choice1", css:map-rend-to-class(.)), ., orig, seg)
                        else
                            if (sic and corr) then
                                printcss:alternate($config, ., ("tei-choice5", css:map-rend-to-class(.)), ., corr[1], sic[1])
                            else
                                if (abbr and expan) then
                                    printcss:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                else
                                    if (orig and reg) then
                                        printcss:alternate($config, ., ("tei-choice7", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(app) return
                        printcss:alternate($config, ., ("tei-app", css:map-rend-to-class(.)), ., lem, rdg)
