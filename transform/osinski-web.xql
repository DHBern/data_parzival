...
module namespace model="http://www.tei-c.org/pm/models/osinski/web";
            ...
            return
                    ...
                    case element(ref) return
                        if (parent::cit or parent::def) then
                            ...
                            html:inline($config, ., ("tei-ref1", "ref-author", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (not(@target)) then
                    ...
                    case element(entryFree) return
                        html:block($config, ., ("tei-entryFree", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(form) return
                        if (starts-with(@facs, 'iiif:')) then
                            html:webcomponent($config, ., ("tei-form", "facs", "columnbreak", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"facs": replace(@facs, '^iiif:([^/]+).*$', '$1'), "coordinates": ('[' || replace(@facs, '^iiif:[^/]+/(.*)$', '$1') || ']'), "emit": 'transcription'})                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(respStmt) return
                        html:alternate($config, ., ("tei-respStmt", css:map-rend-to-class(.)), ., persName, resp, map {})                        => model:map($node, $trackIds)
