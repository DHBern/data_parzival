    ...
    $input !         (
            ...
            return
                    ...
                    case element(lb) return
                        fo:omit($config, ., css:get-rendition(., ("tei-lb", css:map-rend-to-class(.))), .)
                    ...
                    case element(anchor) return
                        fo:note($config, ., ("tei-anchor", css:map-rend-to-class(.)), let $target := '#' || @xml:id return root(.)//div[@type='notes']/note[@target=$target], (), ())
                    ...
                    case element(persName) return
                        if (parent::person) then
                            ...
                            fo:alternate($config, ., ("tei-persName5", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.)))
                    ...
                    case element(placeName) return
                        if (parent::place) then
                            ...
                            fo:alternate($config, ., ("tei-placeName5", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.)))
                    case element(orgName) return
                        if (parent::org) then
                            fo:inline($config, ., ("tei-orgName2", css:map-rend-to-class(.)), .)
                        else
                            fo:alternate($config, ., ("tei-orgName4", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.)))
