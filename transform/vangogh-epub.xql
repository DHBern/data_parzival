...
module namespace model="http://www.tei-c.org/pm/models/vangogh/epub";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?view='metadata') then
                            epub:block($config, ., ("tei-teiHeader5", "metadata", css:map-rend-to-class(.)), .//sourceDesc//vg:letDesc)
                        else
                            if ($parameters?header='short') then
                                epub:block($config, ., ("tei-teiHeader6", css:map-rend-to-class(.)), .)
                            else
                    ...
                    case element(pb) return
                        html:webcomponent($config, ., css:get-rendition(., ("tei-pb3", css:map-rend-to-class(.))), @f || ':' || @n, 'pb-facs-link', map {"facs": let $zone := id(substring-after(@facs, '#'), root($parameters?root)) return  'http://vangoghletters.org/vg/facsimiles/' ||     $zone/graphic[@mimeType="image/png"]/@url, "emit": 'transcription'})
                    ...
                    case element(lb) return
                        if ($parameters?view='normalized') then
                            ...
                            epub:break($config, ., css:get-rendition(., ("tei-lb3", css:map-rend-to-class(.))), ., 'line', @n)
                        ...
                        if ($parameters?view='notes') then
                            ...
                            if (@type='textualNotes') then
                                ...
                                if (@type='title_page') then
                                    epub:block($config, ., ("tei-div7", css:map-rend-to-class(.)), .)
                                else
                                    if (parent::body or parent::front or parent::back) then
                                        html:section($config, ., ("tei-div8", css:map-rend-to-class(.)), .)
                                    else
                                        epub:block($config, ., ("tei-div9", css:map-rend-to-class(.)), .)
                        ...
                        if ((ancestor::div[@type='translation'] or $parameters?view='normalized') and @rend='indent') then
                            ...
                            if (ancestor::div[@type='translation'] or $parameters?view='normalized') then
                                ...
                                if (@rend='indent') then
                                    ...
                                    html:inline($config, ., ("tei-ab6", css:map-rend-to-class(.)), .)
                    ...
                    case element(vg:letDesc) return
                        (
                            ...
                            epub:block($config, ., ("tei-vg_letDesc2", css:map-rend-to-class(.)), note[@type='sourceStatus']/node()),
                            html:heading($config, ., ("tei-vg_letDesc3", css:map-rend-to-class(.)), 'Location:', 3),
                            epub:block($config, ., ("tei-vg_letDesc4", css:map-rend-to-class(.)), note[@type='location']/node()),
                            html:heading($config, ., ("tei-vg_letDesc5", css:map-rend-to-class(.)), 'Date:', 3),
                            epub:block($config, ., ("tei-vg_letDesc6", css:map-rend-to-class(.)), note[@type='date']/node()),
                            html:heading($config, ., ("tei-vg_letDesc7", css:map-rend-to-class(.)), 'Additional:', 3),
                            epub:block($config, ., ("tei-vg_letDesc8", css:map-rend-to-class(.)), note[@type='additionalDetail']/node())
                        )

                    case element(vg:whiteline) return
                        epub:break($config, ., ("tei-vg_whiteline", css:map-rend-to-class(.)), ., 'column', ())
