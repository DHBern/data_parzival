...
module namespace model="http://www.tei-c.org/pm/models/shakespeare/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(fw) return
                        if (@type='rh') then
                            html:block($config, ., ("tei-fw1", "running", css:map-rend-to-class(.)), .)
                        else
                            if (@type) then
                                (: Omit except for running head :)
                                html:omit($config, ., ("tei-fw2", css:map-rend-to-class(.)), .)
                            else
                                if (@type='catchword') then
                                    html:block($config, ., ("tei-fw3", "catchword", css:map-rend-to-class(.)), .)
                                else
                                    if (@type='sig') then
                                        html:block($config, ., ("tei-fw4", "sig", css:map-rend-to-class(.)), .)
                                    else
