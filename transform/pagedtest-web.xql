(:~
    ...
    ODD: /db/apps/parzival/odd/pagedtest.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/pagedtest/web";
...
declare %private function model:template-l($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><span>{$config?apply-children($config, $node, $params?id)}</span><span>{$config?apply-children($config, $node, $params?content)}</span></t>/*
};
    ...
    $input !         (
            ...
            return
                    ...
                    case element(l) return
                        ...
                        return
                                                html:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", css:map-rend-to-class(.))), $content)                        => model:map($node, $trackIds)
