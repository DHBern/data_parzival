(:~
    ...
    ODD: /db/apps/parzival/odd/dantiscus.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/dantiscus/print";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            if ($parameters?mode='title') then
                                html:heading($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), (fileDesc/titleStmt/title[not(@type)]), 5)
                            else
                    ...
                    case element(pb) return
                        if (starts-with(@facs, 'iiif:')) then
                            html:webcomponent($config, ., ("tei-pb1", "facs", css:map-rend-to-class(.)), @n, 'pb-facs-link', map {"emit": 'transcription', "facs": replace(@facs, '^iiif:(.*)$', '$1')})
                        else
                    ...
                    case element(lb) return
                        if ($parameters?mode='norm') then
                            ...
                            html:break($config, ., css:get-rendition(., ("tei-lb2", "lb", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            ...
                            if ($parameters?mode='norm' and abbr and expan) then
                                printcss:alternate($config, ., ("tei-choice5", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                            else
                                if (abbr and expan) then
                                    (
                                        printcss:alternate($config, ., ("tei-choice6", "choice", css:map-rend-to-class(.)), ., abbr[1], expan[1]),
                                        printcss:alternate($config, ., ("tei-choice7", "choice-alternate", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                    )

                                else
                                    if (orig and reg) then
                                        printcss:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(note) return
                        if (parent::person or parent::place) then
                            ...
                            if (@place) then
                                printcss:note($config, ., ("tei-note2", css:map-rend-to-class(.)), ., @place, @n)
                            else
                                if (parent::div and not(@place)) then
                                    html:block($config, ., ("tei-note3", css:map-rend-to-class(.)), .)
                                else
                                    if (not(@place)) then
                    ...
                    case element(foreign) return
                        html:inline($config, ., ("tei-foreign", "foreign", css:map-rend-to-class(.)), .)
                    ...
                    case element(name) return
                        printcss:alternate($config, ., ("tei-name", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node())
                    case element(person) return
                        html:inline($config, ., ("tei-person", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", css:map-rend-to-class(.)), .)
                    case element(rs) return
                        printcss:alternate($config, ., ("tei-rs", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node())
