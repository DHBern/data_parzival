...
module namespace model="http://www.tei-c.org/pm/models/adagia/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(pb) return
                        ...
                        return
                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-pb", css:map-rend-to-class(.))), $content)
                    ...
                    case element(head) return
                        if (@n) then
                            html:heading($config, ., ("tei-head1", css:map-rend-to-class(.)), (@n, '. ', .), count(ancestor::div))
                        else
                            if ($parameters?header='short') then
