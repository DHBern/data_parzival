...
module namespace model="http://www.tei-c.org/pm/models/lelewel/fo";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(lb) return
                        if (@break) then
                            (
                                fo:inline($config, ., ("tei-lb1", "lb-hyphen", css:map-rend-to-class(.)), '-'),
                                fo:break($config, ., ("tei-lb2", "lb", css:map-rend-to-class(.)), ., 'line', @n)
                            )

                        else
                            fo:break($config, ., css:get-rendition(., ("tei-lb3", "lb", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(choice) return
                        if (orig and seg) then
                            fo:alternate($config, ., ("tei-choice1", css:map-rend-to-class(.)), ., orig, seg)
                        else
                            if (sic and corr) then
                                fo:alternate($config, ., ("tei-choice5", css:map-rend-to-class(.)), ., corr[1], sic[1])
                            else
                                if (abbr and expan) then
                                    fo:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                else
                    ...
                    case element(ref) return
                        if (@type="pwn") then
                            fo:link($config, ., ("tei-ref1", "ref-pwn", css:map-rend-to-class(.)), ., @target, map {"target": '_blank'})
                        else
                            if (@type="lelewel") then
                                fo:link($config, ., ("tei-ref2", "ref-lelewel", css:map-rend-to-class(.)), ., @target, map {"target": '_blank'})
                            else
                                if (not(@target)) then
                    ...
                    case element(app) return
                        fo:alternate($config, ., ("tei-app", css:map-rend-to-class(.)), ., lem, rdg)
                    case element(persName) return
                        fo:inline($config, ., ("tei-persName", "index", "person", css:map-rend-to-class(.)), .)
                    case element(placeName) return
                        fo:inline($config, ., ("tei-placeName", "index", "place", css:map-rend-to-class(.)), .)
                    case element() return
