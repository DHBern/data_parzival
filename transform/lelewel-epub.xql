...
module namespace model="http://www.tei-c.org/pm/models/lelewel/epub";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(pb) return
                        if (@facs) then
                            ...
                            if (starts-with(@facs, 'iiif:')) then
                                html:webcomponent($config, ., ("tei-pb2", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"facs": replace(@facs, '^iiif:(.*)$', '$1'), "emit": 'transcription', "label": 'foo'})
                            else
                    ...
                    case element(lb) return
                        if (@break) then
                            (
                                html:inline($config, ., ("tei-lb1", "lb-hyphen", css:map-rend-to-class(.)), '-'),
                                epub:break($config, ., ("tei-lb2", "lb", css:map-rend-to-class(.)), ., 'line', @n)
                            )

                        else
                            epub:break($config, ., css:get-rendition(., ("tei-lb3", "lb", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(choice) return
                        if (orig and seg) then
                            epub:alternate($config, ., ("tei-choice1", css:map-rend-to-class(.)), ., orig, seg)
                        else
                            if (sic and corr) then
                                epub:alternate($config, ., ("tei-choice5", css:map-rend-to-class(.)), ., corr[1], sic[1])
                            else
                                if (abbr and expan) then
                                    epub:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                else
                                    if (orig and reg) then
                                        epub:alternate($config, ., ("tei-choice7", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(ref) return
                        if (@type="pwn") then
                            html:link($config, ., ("tei-ref1", "ref-pwn", css:map-rend-to-class(.)), ., @target, '_blank', map {})
                        else
                            if (@type="lelewel") then
                                html:link($config, ., ("tei-ref2", "ref-lelewel", css:map-rend-to-class(.)), ., @target, '_blank', map {})
                            else
                                if (not(@target)) then
                    ...
                    case element(app) return
                        epub:alternate($config, ., ("tei-app", css:map-rend-to-class(.)), ., lem, rdg)
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", "index", "person", css:map-rend-to-class(.)), .)
                    case element(placeName) return
                        html:inline($config, ., ("tei-placeName", "index", "place", css:map-rend-to-class(.)), .)
