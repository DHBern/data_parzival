...
module namespace model="http://www.tei-c.org/pm/models/ebbe/print";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?mode='metadata') then
                            ...
                            if ($parameters?header='short') then
                                html:block($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), .)
                            else
                    ...
                    case element(ref) return
                        if (starts-with(@target, 'EBBE_')) then
                            html:link($config, ., ("tei-ref1", css:map-rend-to-class(.)), ., 'https://www.estreicher.uj.edu.pl/staropolska/baza/wpis/?sort=nazwisko_imie&amp;order=1&amp;id=' || substring-after(@target, 'EBBE_'), '_ebbe', map {})
                        else
                            if (parent::cit or parent::def) then
                                (: references (authors) within translations, examples or definitions :)
                                html:inline($config, ., ("tei-ref2", "ref-author", css:map-rend-to-class(.)), .)
                            else
                                if (not(@target)) then
                                    printcss:alternate($config, ., ("tei-ref3", css:map-rend-to-class(.)), ., ., 'odsyłacz niezidentyfikowany')
                                else
                                    if (not(node())) then
                                        html:link($config, ., ("tei-ref4", css:map-rend-to-class(.)), @target, @target, (), map {})
                                    else
                                        html:link($config, ., ("tei-ref5", css:map-rend-to-class(.)), ., @target, (), map {})
                    ...
                    case element(ab) return
                        if (starts-with(@facs, 'iiif:')) then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-ab1", "ab", css:map-rend-to-class(.)), $content)
                        else
                            html:block($config, ., ("tei-ab2", "ab", css:map-rend-to-class(.)), .)
                    ...
                    case element(cit) return
                        if (@type='example') then
                            html:block($config, ., ("tei-cit1", "example", css:map-rend-to-class(.)), .)
                        else
                            if (@type='translation') then
                                html:block($config, ., ("tei-cit2", "translation", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                    case element(titleStmt) return
                        if ($parameters?mode='title' ) then
                            ...
                            if ($parameters?mode='metadata') then
                                html:block($config, ., ("tei-titleStmt5", css:map-rend-to-class(.)), .)
                            else
                                if ($parameters?header='short') then
                                    (
                                        html:link($config, ., ("tei-titleStmt6", css:map-rend-to-class(.)), title[1], $parameters?doc, (), map {}),
                                        html:block($config, ., ("tei-titleStmt7", css:map-rend-to-class(.)), subsequence(title, 2)),
                                        html:block($config, ., ("tei-titleStmt8", css:map-rend-to-class(.)), (author, editor))
                                    )

                                else
                                    html:block($config, ., ("tei-titleStmt9", css:map-rend-to-class(.)), .)
                    ...
                    case element(gen) return
                        printcss:alternate($config, ., ("tei-gen", css:map-rend-to-class(.)), ., ., @norm)
                    case element(pos) return
                        printcss:alternate($config, ., ("tei-pos", css:map-rend-to-class(.)), ., ., @norm)
                    case element(editor) return
                        if (ancestor::teiHeader) then
                            html:block($config, ., ("tei-editor1", css:map-rend-to-class(.)), .)
                        else
                            html:inline($config, ., ("tei-editor2", css:map-rend-to-class(.)), .)
                    case element(msDesc) return
                        (
                            (: retrieves the primary language :)
                            html:block($config, ., ("tei-msDesc1", css:map-rend-to-class(.)), (root(.)//langUsage/language)),
                            (: retrieves the form of the source from taxonomy file :)
                            html:block($config, ., ("tei-msDesc2", css:map-rend-to-class(.)), id(substring-after(root(.)//catRef[@scheme='#form']/@target, '#'), doc('/db/apps/tei-publisher/data/taxonomy.xml'))),
                            html:block($config, ., ("tei-msDesc3", css:map-rend-to-class(.)), .)
                        )
                    ...
                    case element(respStmt) return
                        ...
                        return
                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-respStmt", css:map-rend-to-class(.)), $content)
