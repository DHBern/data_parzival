    ...
    $input !         (
            ...
            return
                    ...
                    case element(lb) return
                        if (@break) then
                            (
                                latex:inline($config, ., ("tei-lb1", "lb-hyphen", css:map-rend-to-class(.)), '-'),
                                latex:break($config, ., ("tei-lb2", "lb", css:map-rend-to-class(.)), ., 'line', @n)
                            )

                        else
                            latex:break($config, ., css:get-rendition(., ("tei-lb3", "lb", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(choice) return
                        if (orig and seg) then
                            latex:alternate($config, ., ("tei-choice1", css:map-rend-to-class(.)), ., orig, seg)
                        else
                            if (sic and corr) then
                                latex:alternate($config, ., ("tei-choice5", css:map-rend-to-class(.)), ., corr[1], sic[1])
                            else
                                if (abbr and expan) then
                                    latex:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                else
                                    if (orig and reg) then
                                        latex:alternate($config, ., ("tei-choice7", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(ref) return
                        if (@type="pwn") then
                            latex:link($config, ., ("tei-ref1", "ref-pwn", css:map-rend-to-class(.)), ., @target, map {"target": '_blank'})
                        else
                            if (@type="lelewel") then
                                latex:link($config, ., ("tei-ref2", "ref-lelewel", css:map-rend-to-class(.)), ., @target, map {"target": '_blank'})
                            else
                                if (not(@target)) then
                    ...
                    case element(persName) return
                        latex:inline($config, ., ("tei-persName", "index", "person", css:map-rend-to-class(.)), .)
                    case element(placeName) return
                        latex:inline($config, ., ("tei-placeName", "index", "place", css:map-rend-to-class(.)), .)
                    case element() return
