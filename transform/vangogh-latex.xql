...
module namespace model="http://www.tei-c.org/pm/models/vangogh/latex";
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LO,RE]{\footnotesize `{string-join($config?apply-children($config, $node, $params?title))}`}
\fancyhead[LE,RO]{\footnotesize\thepage}
`{string-join($config?apply-children($config, $node, $params?styles))}`
\setlength{\parindent}{0}
\begin{document}
\title{`{string-join($config?apply-children($config, $node, $params?title))}`}
\date{`{string-join($config?apply-children($config, $node, $params?date))}`}}
\maketitle
    ...
    $input !         (
            ...
            return
                    ...
                    case element(teiHeader) return
                        latex:block($config, ., ("tei-teiHeader1", css:map-rend-to-class(.)), .//sourceDesc//vg:letDesc)
                    ...
                    case element(lb) return
                        ...
                        return
                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-lb1", css:map-rend-to-class(.)), $content)
                    ...
                    case element(TEI) return
                        let $params := 
                            map {
                                ...
                                "date": teiHeader/fileDesc/sourceDesc/vg:letDesc/vg:letHeading/vg:dateLet,
                                "header": teiHeader,
                                "content": .//div[@type='original'],
                                "translation": .//div[@type='translation'],
                        ...
                        return
                                                latex:section(map:merge(($config, map:entry("template", true()))), ., ("tei-TEI1", css:map-rend-to-class(.)), $content)
                    ...
                    case element(note) return
                        if ($parameters?view='notes') then
                            latex:listItem($config, ., ("tei-note1", css:map-rend-to-class(.)), ., @n)
                        else
                    ...
                    case element(div) return
                        if ($parameters?view='notes') then
                            latex:list($config, ., ("tei-div5", css:map-rend-to-class(.)), for $anchor in .//anchor/@n return  root($parameters?root)//div[@type='notes']//note[@n=$anchor], 'custom')
                        else
                            if (@type='textualNotes') then
                                latex:omit($config, ., ("tei-div6", css:map-rend-to-class(.)), .)
                            else
                                if (@type='title_page') then
                                    latex:block($config, ., ("tei-div7", css:map-rend-to-class(.)), .)
                                else
                                    if (parent::body or parent::front or parent::back) then
                                        latex:section($config, ., ("tei-div8", css:map-rend-to-class(.)), .)
                                    else
                                        latex:block($config, ., ("tei-div9", css:map-rend-to-class(.)), .)
                    ...
                    case element(c) return
                        if (@type='kk') then
                            ...
                            if (@type='kp') then
                                ...
                                latex:inline($config, ., ("tei-c3", css:map-rend-to-class(.)), .)
                    case element(vg:letDesc) return
                        (
                            latex:heading($config, ., ("tei-vg_letDesc1", css:map-rend-to-class(.)), 'Source status:', 3),
                            latex:block($config, ., ("tei-vg_letDesc2", css:map-rend-to-class(.)), note[@type='sourceStatus']/node()),
                            latex:heading($config, ., ("tei-vg_letDesc3", css:map-rend-to-class(.)), 'Location:', 3),
                            latex:block($config, ., ("tei-vg_letDesc4", css:map-rend-to-class(.)), note[@type='location']/node()),
                            latex:heading($config, ., ("tei-vg_letDesc5", css:map-rend-to-class(.)), 'Date:', 3),
                            latex:block($config, ., ("tei-vg_letDesc6", css:map-rend-to-class(.)), note[@type='date']/node()),
                            latex:heading($config, ., ("tei-vg_letDesc7", css:map-rend-to-class(.)), 'Additional:', 3),
