(:~
    ...
    ODD: /db/apps/parzival/odd/lelewel.odd
 :)
xquery version "3.1";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(pb) return
                        if (@facs) then
                            ...
                            if (starts-with(@facs, 'iiif:')) then
                                html:webcomponent($config, ., ("tei-pb2", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"facs": replace(@facs, '^iiif:(.*)$', '$1'), "emit": 'transcription', "label": 'foo'})                                => model:map($node, $trackIds)
                            else
                    ...
                    case element(lb) return
                        if (@break) then
                            (
                                html:inline($config, ., ("tei-lb1", "lb-hyphen", css:map-rend-to-class(.)), '-')                                => model:map($node, $trackIds),
                                html:break($config, ., ("tei-lb2", "lb", css:map-rend-to-class(.)), ., 'line', @n)                                => model:map($node, $trackIds)
                            )

                        else
                            html:break($config, ., css:get-rendition(., ("tei-lb3", "lb", css:map-rend-to-class(.))), ., 'line', @n)                            => model:map($node, $trackIds)
                    ...
                    case element(choice) return
                        if (orig and seg) then
                            html:alternate($config, ., ("tei-choice1", css:map-rend-to-class(.)), ., orig, seg, map {})                            => model:map($node, $trackIds)
                        else
                            if (sic and corr) then
                                html:alternate($config, ., ("tei-choice5", css:map-rend-to-class(.)), ., corr[1], sic[1], map {})                                => model:map($node, $trackIds)
                            else
                                if (abbr and expan) then
                                    html:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., expan[1], abbr[1], map {})                                    => model:map($node, $trackIds)
                                else
                    ...
                    case element(ref) return
                        if (@type="pwn") then
                            html:link($config, ., ("tei-ref1", "ref-pwn", css:map-rend-to-class(.)), ., @target, '_blank', map {})                            => model:map($node, $trackIds)
                        else
                            if (@type="lelewel") then
                                html:link($config, ., ("tei-ref2", "ref-lelewel", css:map-rend-to-class(.)), ., @target, '_blank', map {})                                => model:map($node, $trackIds)
                            else
                                if (not(@target)) then
                    ...
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", "index", "person", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(placeName) return
                        html:inline($config, ., ("tei-placeName", "index", "place", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
