...
module namespace model="http://www.tei-c.org/pm/models/potocki/latex";
            ...
            return
                    ...
                    case element(l) return
                        if (starts-with(@facs, 'iiif:')) then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-l1", "verse", css:map-rend-to-class(.)), $content)
                        else
                            latex:block($config, ., css:get-rendition(., ("tei-l2", "verse", css:map-rend-to-class(.))), .)
                    ...
                    case element(choice) return
                        if (parent::pc) then
                            (
                                latex:alternate($config, ., ("tei-choice1", "pc-choice", css:map-rend-to-class(.)), ., *[2], *[1]),
                                latex:alternate($config, ., ("tei-choice2", "pc-choice-alternate", css:map-rend-to-class(.)), ., *[1], *[2])
                            )

                        else
                            (
                                latex:alternate($config, ., ("tei-choice3", "choice", css:map-rend-to-class(.)), ., *[2], *[1]),
                                latex:alternate($config, ., ("tei-choice4", "choice-alternate", css:map-rend-to-class(.)), ., *[1], *[2])
                            )
                    ...
                    case element(note) return
                        if (@type='ling') then
                            ...
                            return
                                                        latex:note(map:merge(($config, map:entry("template", true()))), ., ("tei-note1", "ling", css:map-rend-to-class(.)), $content, (), ())
                        else
                            if ($parameters?view="introduction" and @type="introduction") then
                                ...
                                return
                                                                latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-note2", css:map-rend-to-class(.)), $content)
                            else
                                if (@type="introduction") then
                                    (: omit in the reading panel :)
                                    latex:omit($config, ., ("tei-note3", "introduction", css:map-rend-to-class(.)), .)
                                else
                    ...
                    case element(div) return
                        if ($parameters?view='introduction') then
                            latex:block($config, ., ("tei-div1", css:map-rend-to-class(.)), .//note[@type='introduction'])
                        else
                    ...
                    case element(graphic) return
                        latex:graphic($config, ., ("tei-graphic", "graphic", css:map-rend-to-class(.)), ., @url, @width, @height, @scale, caption)
                    case element(ref) return
                        if (@corresp and @target) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-ref1", css:map-rend-to-class(.)), $content)
                        else
                            if (not(@target)) then
                                ...
                                if (not(node())) then
                                    ...
                                    latex:link($config, ., ("tei-ref4", css:map-rend-to-class(.)), ., @target, map {"target": '_blank'})
                    ...
                    case element(quote) return
                        latex:inline($config, ., css:get-rendition(., ("tei-quote", "quote", css:map-rend-to-class(.))), .)
                    ...
                    case element(person) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person", "person", css:map-rend-to-class(.)), $content)
                    case element(distinct) return
                        if (note) then
                            latex:inline($config, ., ("tei-distinct1", "distinct", css:map-rend-to-class(.)), .)
                        else
                            if (note) then
                                (
                                    latex:inline($config, ., ("tei-distinct2", "distinct", css:map-rend-to-class(.)), seg),
                                    if (note) then
                                        latex:note($config, ., ("tei-distinct3", css:map-rend-to-class(.)), note, (), ())
                                    else
                                        ()
                                )
