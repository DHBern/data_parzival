...
module namespace model="http://www.tei-c.org/pm/models/time-us/web";
...
declare %private function model:template-teiHeader3($config as map(*), $node as node()*, $params as map(*)) {
    ``[Editor : `{string-join($config?apply-children($config, $node, $params?content))}`]``
};
(: generated template function for element spec: teiHeader :)
...
(: generated template function for element spec: w :)
declare %private function model:template-w($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><token name="{$config?apply-children($config, $node, $params?name)}" type="{$config?apply-children($config, $node, $params?type)}" kind="{$config?apply-children($config, $node, $params?kind)}" gender="{$config?apply-children($config, $node, $params?gender)}" id="{$config?apply-children($config, $node, $params?id)}">{$config?apply-children($config, $node, $params?content)}</token></t>/*
};
...
declare %private function model:template-respStmt($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><li>{$config?apply-children($config, $node, $params?task)} : {$config?apply-children($config, $node, $params?person)}</li></t>/*
};
(:~
    ...
    $input !         (
            ...
            return
                        ...
                        if ($parameters?header='short') then
                            ...
                            if ($parameters?mode="metadata") then
                                (
                                    let $params := 
                                        map {
                                            "content": fileDesc/sourceDesc/p/idno[@type='URI']
                                        }

                                                                        let $content := 
                                        model:template-teiHeader2($config, ., $params)
                                    return
                                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader2", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds),
                                    let $params := 
                                        map {
                                            "content": fileDesc/titleStmt/editor/text()
                                        }

                                                                        let $content := 
                                        model:template-teiHeader3($config, ., $params)
                                    return
                                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader3", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds),
                                    let $params := 
                                        map {
                                            "content": .
                                        }
                                    ...
                                    return
                                                                        html:text(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader4", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds),
                                    let $params := 
                                        map {
                                            "content": fileDesc/titleStmt/respStmt
                                        }

                                                                        let $content := 
                                        model:template-teiHeader5($config, ., $params)
                                    return
                                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader5", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds)
                                )

                            else
                                if ($parameters?mode='title') then
                                    html:inline($config, ., ("tei-teiHeader6", css:map-rend-to-class(.)), (fileDesc/titleStmt/title[1]))                                    => model:map($node, $trackIds)
                                else
                                    html:metadata($config, ., ("tei-teiHeader9", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                    ...
                    case element(p) return
                        if (parent::publicationStmt) then
                    ...
                    case element(pb) return
                        html:omit($config, ., css:get-rendition(., ("tei-pb", css:map-rend-to-class(.))), .)                        => model:map($node, $trackIds)
                    ...
                    case element(w) return
                        let $params := 
                            map {
                                "content": .,
                                "id": @xml:id,
                                "type": let $i:=@xml:id  let $pattern:=concat('#', $i, "(\s|$)") return root(.)/TEI/standOff/entities//span[matches(@target, $pattern)]/../../@type,
                                "name": let $i:=@xml:id  let $pattern:=concat('#', $i, "(\s|$)") return root(.)/TEI/standOff/entities//span[matches(@target, $pattern)]/../../@name,
                                "kind": let $i:=@xml:id  let $pattern:=concat('#', $i, "(\s|$)") return root(.)/TEI/standOff/concepts//span[matches(@target, $pattern)]/../../@kind,
                                "gender": let $i:=@xml:id  let $pattern:=concat('#', $i, "(\s|$)") return root(.)/TEI/standOff/entities//span[matches(@target, $pattern)]/../../@gender
                            }

                                                let $content := 
                            model:template-w($config, ., $params)
                        return
                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-w", css:map-rend-to-class(.)), $content)                        => model:map($node, $trackIds)
                    ...
                    case element(publicationStmt) return
                        if ($parameters?header='short') then
                            html:block($config, ., ("tei-publicationStmt1", css:map-rend-to-class(.)), p)                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(fileDesc) return
                        if ($parameters?header='short') then
                            (
                                ...
                                html:block($config, ., ("tei-fileDesc2", "header-short", css:map-rend-to-class(.)), sourceDesc)                                => model:map($node, $trackIds),
                    ...
                    case element(body) return
                        (
                            ...
                            html:inline($config, ., ("tei-body2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        )
                    ...
                    case element(text) return
                        html:body($config, ., ("tei-text", css:map-rend-to-class(.)), (root(.)//entities, .))                        => model:map($node, $trackIds)
                    ...
                    case element(respStmt) return
                        if ($parameters?mode="metadata") then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-respStmt", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
                    case element(sourceDesc) return
                        if ($parameters?header='short') then
                            html:link($config, ., ("tei-sourceDesc", "header-short", css:map-rend-to-class(.)), "Facsimile", p/idno[@type="URI"], "_blank", map {})                            => model:map($node, $trackIds)
                        else
                            $config?apply($config, ./node())
