...
module namespace model="http://www.tei-c.org/pm/models/parzival-verse-inline/web";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(l) return
                        html:inline($config, ., css:get-rendition(., ("tei-l", "test", css:map-rend-to-class(.))), .)                        => model:map($node, $trackIds)
                    ...
                    case element(seg) return
                        html:inline($config, ., ("tei-seg1", "glory-initial", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
