(:~
    ...
    ODD: /db/apps/parzival/odd/adagia.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/adagia/web";
...
declare %private function model:template-pb($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><pb-facs-link facs="{$config?apply-children($config, $node, $params?facs)}" emit="transcription"><div class="facscaption">Źródło: <a href="https://rcin.org.pl/dlibra/publication/74578/edition/66983/content">
IBL PAN, call no. XVI.3.3 </a> {$config?apply-children($config, $node, $params?content)} 
                            </div></pb-facs-link></t>/*
};
    ...
    $input !         (
            ...
            return
                    ...
                    case element(pb) return
                        ...
                        return
                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-pb", css:map-rend-to-class(.))), $content)                        => model:map($node, $trackIds)
                    ...
                    case element(head) return
                        if (@n) then
                            html:heading($config, ., ("tei-head1", css:map-rend-to-class(.)), (@n, '. ', .), count(ancestor::div))                            => model:map($node, $trackIds)
                        else
                            if ($parameters?header='short') then
                                html:inline($config, ., ("tei-head2", css:map-rend-to-class(.)), replace(string-join(.//text()[not(parent::ref)]), '^(.*?)[^\w]*$', '$1'))                                => model:map($node, $trackIds)
                            else
                                    ...
                                    if (parent::table) then
                                        ...
                                        if (parent::lg) then
                                            ...
                                            if (parent::list) then
                                                ...
                                                if (parent::div) then
                                                    html:heading($config, ., ("tei-head7", css:map-rend-to-class(.)), ., count(ancestor::div))                                                    => model:map($node, $trackIds)
                                                else
