...
module namespace model="http://www.tei-c.org/pm/models/docx-output/fo";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(supplied) return
                        fo:inline($config, ., ("tei-supplied", css:map-rend-to-class(.)), .)
                    ...
                    case element(anchor) return
                        if (@type='note') then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-anchor1", "note", css:map-rend-to-class(.)), $content)
                        else
                    ...
                    case element(hi) return
                        if (@rend) then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-hi1", css:map-rend-to-class(.)), $content)
                        else
                            if (@rendition) then
                    ...
                    case element(note) return
                        if (@target) then
                            fo:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., (), let $nr := count(./preceding::note[@target]) let $ch := codepoints-to-string(string-to-codepoints("a") + $nr mod 26) return  '-' || $ch)
                        else
                            if (@place) then
                        ...
                        if (not(@target)) then
                            ...
                            if (not(node())) then
                                ...
                                if (starts-with(@target, '#')) then
                                    fo:link($config, ., ("tei-ref3", css:map-rend-to-class(.)), ., '?id=' || substring-after(@target, '#') || @target, map {})
                                else
                                ...
                                if (parent::table) then
                                    ...
                                    if (parent::lg) then
                                        ...
                                        if (parent::list) then
                                            ...
                                            if (parent::div) then
                                                fo:heading($config, ., ("tei-head6", css:map-rend-to-class(.)), ., count($get(.)/ancestor::div))
                                            else
                    ...
                    case element(placeName) return
                        if (@rend='smallcaps') then
                            fo:link($config, ., ("tei-placeName1", css:map-rend-to-class(.)), ., 'https://www.google.de/maps/place/' || @ref, map {"target": '_blank'})
                        else
                            fo:inline($config, ., ("tei-placeName2", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        if (@ref) then
                            fo:link($config, ., ("tei-persName1", css:map-rend-to-class(.)), ., @ref, map {"target": '_blank'})
                        else
                            fo:inline($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                    case element(tag) return
                        fo:inline($config, ., ("tei-tag", css:map-rend-to-class(.)), .)
