(:~
    ...
    ODD: /db/apps/parzival/odd/dta.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/dta/web";
            ...
            return
                    ...
                    case element(pb) return
                        if (count(../*) = 1 and count(ancestor::*) = 1) then
                            ...
                            html:webcomponent($config, ., ("tei-pb3", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"emit": 'transcription', "facs": let $name := replace(util:document-name($parameters?root), '^([^\.]+)\..*$', '$1') return  ``[`{$name}`/`{$name}`_`{substring-after(@facs, '#f')}`_800px.jpg]``})                            => model:map($node, $trackIds)
                    ...
                    case element(lb) return
                        if (@break='no' and $parameters?view='page') then
                            (
                                html:inline($config, ., ("tei-lb1", css:map-rend-to-class(.)), '-')                                => model:map($node, $trackIds),
                                html:break($config, ., ("tei-lb2", css:map-rend-to-class(.)), ., 'line', @n)                                => model:map($node, $trackIds)
                            )

                        else
                            if ($parameters?view='page' and not(following-sibling::*[1][self::p|self::div])) then
