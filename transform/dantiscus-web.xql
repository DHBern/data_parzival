    ...
    $input !         (
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='short') then
                            ...
                            if ($parameters?mode='title') then
                                html:heading($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), (fileDesc/titleStmt/title[not(@type)]), 5)                                => model:map($node, $trackIds)
                            else
                            ...
                            if ($parameters?mode='norm' and abbr and expan) then
                                ...
                                if (abbr and expan) then
                                    (
                                        ...
                                        html:alternate($config, ., ("tei-choice7", "choice-alternate", css:map-rend-to-class(.)), ., expan[1], abbr[1], map {})                                        => model:map($node, $trackIds)
                                    )

                                else
                                    if (orig and reg) then
                                        html:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1], map {})                                        => model:map($node, $trackIds)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(foreign) return
                        html:inline($config, ., ("tei-foreign", "foreign", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(name) return
                        html:alternate($config, ., ("tei-name", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node(), map {})                        => model:map($node, $trackIds)
                    case element(person) return
                        html:inline($config, ., ("tei-person", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(rs) return
                        html:alternate($config, ., ("tei-rs", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node(), map {})                        => model:map($node, $trackIds)
