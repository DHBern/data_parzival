...
module namespace model="http://www.tei-c.org/pm/models/annotations/web";
            ...
            return
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            ...
                            return
                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-choice1", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            if (abbr and expan) then
                                ...
                                return
                                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-choice2", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                if (orig and reg) then
                                    ...
                                    return
                                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-choice3", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds)
                                else
                                    $config?apply($config, ./node())
                    case element(role) return
                        html:block($config, ., ("tei-role", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(hi) return
                        html:inline($config, ., css:get-rendition(., ("tei-hi", "annotation", "annotation-hi", css:map-rend-to-class(.))), .)                        => model:map($node, $trackIds)
                    ...
                    case element(ref) return
                        html:inline($config, ., ("tei-ref", "annotation", "annotation-ref", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(date) return
                        html:inline($config, ., ("tei-date", "annotation", "annotation-date", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(app) return
                        ...
                        return
                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-app", css:map-rend-to-class(.)), $content)                        => model:map($node, $trackIds)
