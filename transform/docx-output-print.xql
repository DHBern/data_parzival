(:~
    ...
    ODD: /db/apps/parzival/odd/docx-output.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/docx-output/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(supplied) return
                        html:inline($config, ., ("tei-supplied", css:map-rend-to-class(.)), .)
                    ...
                    case element(anchor) return
                        if (@type='note') then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-anchor1", "note", css:map-rend-to-class(.)), $content)
                        else
                    ...
                    case element(hi) return
                        if (@rend) then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-hi1", css:map-rend-to-class(.)), $content)
                        else
                            if (@rendition) then
                    ...
                    case element(note) return
                        if (@target) then
                            printcss:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., (), let $nr := count(./preceding::note[@target]) let $ch := codepoints-to-string(string-to-codepoints("a") + $nr mod 26) return  '-' || $ch)
                        else
                            if (@place) then
                        ...
                        if (not(@target)) then
                            ...
                            if (not(node())) then
                                ...
                                if (starts-with(@target, '#')) then
                                    html:link($config, ., ("tei-ref3", css:map-rend-to-class(.)), ., '?id=' || substring-after(@target, '#') || @target, (), map {})
                                else
                                ...
                                if (parent::table) then
                                    ...
                                    if (parent::lg) then
                                        ...
                                        if (parent::list) then
                                            ...
                                            if (parent::div) then
                                                html:heading($config, ., ("tei-head6", css:map-rend-to-class(.)), ., count($get(.)/ancestor::div))
                                            else
                    ...
                    case element(gi) return
                        html:inline($config, ., ("tei-gi", css:map-rend-to-class(.)), .)
                    case element(placeName) return
                        if (@rend='smallcaps') then
                            html:link($config, ., ("tei-placeName1", css:map-rend-to-class(.)), ., 'https://www.google.de/maps/place/' || @ref, '_blank', map {})
                        else
                            html:inline($config, ., ("tei-placeName2", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        if (@ref) then
                            html:link($config, ., ("tei-persName1", css:map-rend-to-class(.)), ., @ref, '_blank', map {})
                        else
                            html:inline($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                    case element(tag) return
                        html:inline($config, ., ("tei-tag", css:map-rend-to-class(.)), .)
                    case element(eg) return
                        html:block($config, ., ("tei-eg", css:map-rend-to-class(.)), .)
