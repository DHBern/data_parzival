(:~
    ...
    ODD: /db/apps/parzival/odd/graves.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/graves/print";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(closer) return
                        html:block($config, ., ("tei-closer2", css:map-rend-to-class(.)), .)
                    ...
                    case element(postscript) return
                        (
                            html:heading($config, ., ("tei-postscript2", css:map-rend-to-class(.)), ('Postscript by ', id(substring-after(@resp, '#'), root(.))/persName), 5),
                            html:block($config, ., ("tei-postscript3", css:map-rend-to-class(.)), .)
                        )

                    case element(dateline) return
                        html:block($config, ., css:get-rendition(., ("tei-dateline2", css:map-rend-to-class(.))), .)
                    ...
                    case element(opener) return
                        html:block($config, ., ("tei-opener2", css:map-rend-to-class(.)), .)
                    ...
                    case element(body) return
                        if ($parameters?mode='facets') then
                            (
                                ...
                                html:block($config, ., ("tei-body2", css:map-rend-to-class(.)), for $n in .//name[@type='place'] group by $ref := $n/@ref order by $ref return $n[1]),
                            ...
                            (
                                html:index($config, ., ("tei-body5", css:map-rend-to-class(.)), 'toc', .),
                                html:block($config, ., ("tei-body6", css:map-rend-to-class(.)), .)
                            )
                            ...
                            if ($parameters?mode='facets' and @type='place') then
                                ...
                                if (@type='place' and id(substring-after(@ref, '#'), root($parameters?root))/location/geo) then
                                    ...
                                    if (@type='person' and id(substring-after(@ref, '#'), root($parameters?root))) then
                                        ...
                                        if (@type='place') then
                                            html:inline($config, ., ("tei-name6", css:map-rend-to-class(.)), .)
                                        else
                                            $config?apply($config, ./node())
                    case element(place) return
                        (
                            ...
                            if (location/geo) then
                                html:block($config, ., ("tei-place3", css:map-rend-to-class(.)), location/geo)
                            else
                                (),
                            html:block($config, ., ("tei-place4", css:map-rend-to-class(.)), string-join(location/* except location/geo, ', ')),
                            html:block($config, ., ("tei-place5", css:map-rend-to-class(.)), note/node())
                        )
                    ...
                    case element(person) return
                        (
                            html:heading($config, ., ("tei-person2", css:map-rend-to-class(.)), persName, 3),
                            if (birth or death or occupation) then
                                html:block($config, ., ("tei-person3", css:map-rend-to-class(.)), (occupation, birth, death))
                            else
                                (),
                            if (idno) then
                                html:block($config, ., ("tei-person4", css:map-rend-to-class(.)), idno)
                            else
                                (),
                            html:block($config, ., ("tei-person5", css:map-rend-to-class(.)), note/node())
                        )

                    case element(persName) return
                        if (parent::person and (forename or surname)) then
                            html:inline($config, ., ("tei-persName1", css:map-rend-to-class(.)), (forename, ' ', surname[not(@type='married')], if (surname[@type='married']) then (' (', string-join(surname[@type='married'], ', '), ')') else ()))
                        else
                            if (parent::person) then
                                html:inline($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                            else
                                printcss:alternate($config, ., ("tei-persName3", css:map-rend-to-class(.)), ., ., id(@ref, doc($global:register-root || "/persons.xml")))
                    case element(birth) return
                        if (following-sibling::death) then
                            html:inline($config, ., ("tei-birth1", css:map-rend-to-class(.)), ('* ', ., '; '))
                        else
                            html:inline($config, ., ("tei-birth2", css:map-rend-to-class(.)), ('* ', .))
                    case element(death) return
                        html:inline($config, ., ("tei-death", css:map-rend-to-class(.)), ('✝', .))
                    case element(occupation) return
                        html:inline($config, ., ("tei-occupation", css:map-rend-to-class(.)), (., ' '))
                    case element(idno) return
                        if (@type='VIAF' and following-sibling::idno) then
                            html:link($config, ., ("tei-idno1", css:map-rend-to-class(.)), 'VIAF', 'https://viaf.org/viaf/' || string() || '/', (), map {})
                        else
                            if (@type='VIAF') then
                                html:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), 'VIAF', 'https://viaf.org/viaf/' || string() || '/', (), map {})
                            else
                                if (@type='LC-Name-Authority-File' and following-sibling::idno) then
                                    html:link($config, ., ("tei-idno3", css:map-rend-to-class(.)), 'LoC Authority', 'https://lccn.loc.gov/' || string(), (), map {})
                                else
                                    if (@type='LC-Name-Authority-File') then
                                        html:link($config, ., ("tei-idno4", css:map-rend-to-class(.)), 'LoC Authority', 'https://lccn.loc.gov/' || string(), (), map {})
                                    else
                                        $config?apply($config, ./node())
                    case element(placeName) return
                        printcss:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id(@ref, doc($global:register-root || "/places.xml")))
                    case element(term) return
                        printcss:alternate($config, ., ("tei-term", css:map-rend-to-class(.)), ., ., id(@ref, doc($global:register-root || "/keywords.xml")))
