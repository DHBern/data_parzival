    ...
    $input !         (
            ...
            return
                    ...
                    case element(note) return
                        if (@target) then
                            epub:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., (), let $nr := count(./preceding::note[@target]) let $ch := codepoints-to-string(string-to-codepoints("a") + $nr mod 26) return  '-' || $ch)
                        else
                            if (@place) then
                    ...
                    case element(eg) return
                        epub:block($config, ., ("tei-eg", css:map-rend-to-class(.)), .)
