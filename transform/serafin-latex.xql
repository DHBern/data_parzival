    ...
    $input !         (
            ...
            return
                    ...
                    case element(lb) return
                        latex:omit($config, ., css:get-rendition(., ("tei-lb", css:map-rend-to-class(.))), .)
                    ...
                    case element(anchor) return
                        latex:note($config, ., ("tei-anchor", css:map-rend-to-class(.)), let $target := '#' || @xml:id return root(.)//div[@type='notes']/note[@target=$target], (), ())
