    ...
    $input !         (
            ...
            return
                    ...
                    case element(closer) return
                        fo:block($config, ., ("tei-closer2", css:map-rend-to-class(.)), .)
                    ...
                    case element(postscript) return
                        (
                            fo:heading($config, ., ("tei-postscript2", css:map-rend-to-class(.)), ('Postscript by ', id(substring-after(@resp, '#'), root(.))/persName), 5),
                            fo:block($config, ., ("tei-postscript3", css:map-rend-to-class(.)), .)
                        )

                    case element(dateline) return
                        fo:block($config, ., css:get-rendition(., ("tei-dateline2", css:map-rend-to-class(.))), .)
                    ...
                    case element(opener) return
                        fo:block($config, ., ("tei-opener2", css:map-rend-to-class(.)), .)
                    ...
                    case element(body) return
                        if ($parameters?mode='facets') then
                            ...
                            (
                                fo:index($config, ., ("tei-body5", css:map-rend-to-class(.)), ., 'toc'),
                                fo:block($config, ., ("tei-body6", css:map-rend-to-class(.)), .)
                            )
                            ...
                            if ($parameters?mode='facets' and @type='place') then
                                ...
                                if (@type='place' and id(substring-after(@ref, '#'), root($parameters?root))/location/geo) then
                                    ...
                                    if (@type='person' and id(substring-after(@ref, '#'), root($parameters?root))) then
                                        ...
                                        if (@type='place') then
                                            fo:inline($config, ., ("tei-name6", css:map-rend-to-class(.)), .)
                                        else
                                            $config?apply($config, ./node())
                    case element(place) return
                        (
                            ...
                            if (location/geo) then
                                ...
                                (),
                            fo:block($config, ., ("tei-place4", css:map-rend-to-class(.)), string-join(location/* except location/geo, ', ')),
                            fo:block($config, ., ("tei-place5", css:map-rend-to-class(.)), note/node())
                        )
                    ...
                    case element(person) return
                        (
                            fo:heading($config, ., ("tei-person2", css:map-rend-to-class(.)), persName, 3),
                            if (birth or death or occupation) then
                                fo:block($config, ., ("tei-person3", css:map-rend-to-class(.)), (occupation, birth, death))
                            else
                                (),
                            if (idno) then
                                fo:block($config, ., ("tei-person4", css:map-rend-to-class(.)), idno)
                            else
                                (),
                            fo:block($config, ., ("tei-person5", css:map-rend-to-class(.)), note/node())
                        )

                    case element(persName) return
                        if (parent::person and (forename or surname)) then
                            fo:inline($config, ., ("tei-persName1", css:map-rend-to-class(.)), (forename, ' ', surname[not(@type='married')], if (surname[@type='married']) then (' (', string-join(surname[@type='married'], ', '), ')') else ()))
                        else
                    ...
                    case element(birth) return
                        if (following-sibling::death) then
                            ...
                            fo:inline($config, ., ("tei-birth2", css:map-rend-to-class(.)), ('* ', .))
                    case element(death) return
                        fo:inline($config, ., ("tei-death", css:map-rend-to-class(.)), ('✝', .))
                    case element(occupation) return
                        fo:inline($config, ., ("tei-occupation", css:map-rend-to-class(.)), (., ' '))
                    case element(idno) return
                        if (@type='VIAF' and following-sibling::idno) then
                            fo:link($config, ., ("tei-idno1", css:map-rend-to-class(.)), 'VIAF', 'https://viaf.org/viaf/' || string() || '/', map {})
                        else
                            if (@type='VIAF') then
                                fo:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), 'VIAF', 'https://viaf.org/viaf/' || string() || '/', map {})
                            else
                                if (@type='LC-Name-Authority-File' and following-sibling::idno) then
                                    fo:link($config, ., ("tei-idno3", css:map-rend-to-class(.)), 'LoC Authority', 'https://lccn.loc.gov/' || string(), map {})
                                else
                                    if (@type='LC-Name-Authority-File') then
                                        fo:link($config, ., ("tei-idno4", css:map-rend-to-class(.)), 'LoC Authority', 'https://lccn.loc.gov/' || string(), map {})
                                    else
                                        $config?apply($config, ./node())
