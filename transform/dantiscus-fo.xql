...
module namespace model="http://www.tei-c.org/pm/models/dantiscus/fo";
    ...
    $input !         (
            ...
            return
                            ...
                            if ($parameters?mode='norm' and abbr and expan) then
                                ...
                                if (abbr and expan) then
                                    (
                                        ...
                                        fo:alternate($config, ., ("tei-choice7", "choice-alternate", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                    )

                                else
                                    if (orig and reg) then
                                        fo:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(foreign) return
                        fo:inline($config, ., ("tei-foreign", "foreign", css:map-rend-to-class(.)), .)
                    ...
                    case element(name) return
                        fo:alternate($config, ., ("tei-name", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node())
                    case element(person) return
                        fo:inline($config, ., ("tei-person", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        fo:inline($config, ., ("tei-persName", css:map-rend-to-class(.)), .)
                    case element(rs) return
                        fo:alternate($config, ., ("tei-rs", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node())
