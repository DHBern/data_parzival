...
module namespace model="http://www.tei-c.org/pm/models/vangogh/web";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?view='metadata') then
                            html:block($config, ., ("tei-teiHeader5", "metadata", css:map-rend-to-class(.)), .//sourceDesc//vg:letDesc)                            => model:map($node, $trackIds)
                        else
                            if ($parameters?header='short') then
                                html:block($config, ., ("tei-teiHeader6", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                if ($parameters?header='letter') then
                                    html:inline($config, ., ("tei-teiHeader7", css:map-rend-to-class(.)), fileDesc/titleStmt/title)                                    => model:map($node, $trackIds)
                                else
                    ...
                    case element(pb) return
                        html:webcomponent($config, ., css:get-rendition(., ("tei-pb3", css:map-rend-to-class(.))), @f || ':' || @n, 'pb-facs-link', map {"facs": let $zone := id(substring-after(@facs, '#'), root($parameters?root)) return  'http://vangoghletters.org/vg/facsimiles/' ||     $zone/graphic[@mimeType="image/png"]/@url, "emit": 'transcription'})                        => model:map($node, $trackIds)
                    ...
                    case element(lb) return
                        if ($parameters?view='normalized') then
                            ...
                            html:break($config, ., css:get-rendition(., ("tei-lb3", css:map-rend-to-class(.))), ., 'line', @n)                            => model:map($node, $trackIds)
                    ...
                    case element(TEI) return
                        html:document($config, ., ("tei-TEI3", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(note) return
                        if ($parameters?view='notes') then
                            html:listItem($config, ., ("tei-note1", css:map-rend-to-class(.)), ., @n)                            => model:map($node, $trackIds)
                        else
                            html:block($config, ., ("tei-note2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    ...
                    case element(div) return
                        if ($parameters?view='notes') then
                            html:list($config, ., ("tei-div5", css:map-rend-to-class(.)), for $anchor in .//anchor/@n return  root($parameters?root)//div[@type='notes']//note[@n=$anchor], 'custom')                            => model:map($node, $trackIds)
                        else
                            if (@type='textualNotes') then
                                html:omit($config, ., ("tei-div6", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                if (@type='title_page') then
                                    html:block($config, ., ("tei-div7", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                                else
                                    if (parent::body or parent::front or parent::back) then
                                        html:section($config, ., ("tei-div8", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                                    else
                                        html:block($config, ., ("tei-div9", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                    ...
                    case element(ab) return
                        if ((ancestor::div[@type='translation'] or $parameters?view='normalized') and @rend='indent') then
                            html:paragraph($config, ., ("tei-ab3", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (ancestor::div[@type='translation'] or $parameters?view='normalized') then
                                html:paragraph($config, ., ("tei-ab4", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                if (@rend='indent') then
                                    html:inline($config, ., ("tei-ab5", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                                else
                                    html:inline($config, ., ("tei-ab6", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                    ...
                    case element(c) return
                        if (@type='kk') then
                            html:inline($config, ., ("tei-c1", css:map-rend-to-class(.)), ',')                            => model:map($node, $trackIds)
                        else
                            if (@type='kp') then
                                html:inline($config, ., ("tei-c2", css:map-rend-to-class(.)), '.')                                => model:map($node, $trackIds)
                            else
                                html:inline($config, ., ("tei-c3", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                    case element(vg:letDesc) return
                        (
                            html:heading($config, ., ("tei-vg_letDesc1", css:map-rend-to-class(.)), 'Source status:', 3)                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-vg_letDesc2", css:map-rend-to-class(.)), note[@type='sourceStatus']/node())                            => model:map($node, $trackIds),
                            html:heading($config, ., ("tei-vg_letDesc3", css:map-rend-to-class(.)), 'Location:', 3)                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-vg_letDesc4", css:map-rend-to-class(.)), note[@type='location']/node())                            => model:map($node, $trackIds),
                            html:heading($config, ., ("tei-vg_letDesc5", css:map-rend-to-class(.)), 'Date:', 3)                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-vg_letDesc6", css:map-rend-to-class(.)), note[@type='date']/node())                            => model:map($node, $trackIds),
                            html:heading($config, ., ("tei-vg_letDesc7", css:map-rend-to-class(.)), 'Additional:', 3)                            => model:map($node, $trackIds),
                            html:block($config, ., ("tei-vg_letDesc8", css:map-rend-to-class(.)), note[@type='additionalDetail']/node())                            => model:map($node, $trackIds)
                        )

                    case element(vg:whiteline) return
                        html:break($config, ., ("tei-vg_whiteline", css:map-rend-to-class(.)), ., 'column', ())                        => model:map($node, $trackIds)
