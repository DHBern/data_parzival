...
module namespace model="http://www.tei-c.org/pm/models/ebbe/epub";
            ...
            return
                    ...
                    case element(ref) return
                        if (starts-with(@target, 'EBBE_')) then
                            ...
                            if (parent::cit or parent::def) then
                                ...
                                if (not(@target)) then
                                    epub:alternate($config, ., ("tei-ref3", css:map-rend-to-class(.)), ., ., 'odsyłacz niezidentyfikowany')
                                else
                                    if (not(node())) then
                    ...
                    case element(ab) return
                        if (starts-with(@facs, 'iiif:')) then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-ab1", "ab", css:map-rend-to-class(.)), $content)
                        else
                            epub:block($config, ., ("tei-ab2", "ab", css:map-rend-to-class(.)), .)
